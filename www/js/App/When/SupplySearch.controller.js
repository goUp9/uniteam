App.controller('SupplySearchCtrl', function($http,$scope, Get, Spinner){
        var self=this;
...
//                                window.console.log(data); 
        ...
        this.submit=function(){
            ...
            $http({
                    method  : 'POST',
                    url     : 'http://'+document.domain+'/ajax/supply/when/',
                    data    : $.param(postData),
                    headers : { 'Content-Type': 'application/x-www-form-urlencoded','X-Requested-With':'XMLHttpRequest' }
                })
                    .success(function(data) {
                        if(typeof(Get.bag['query_id'])==='undefined'){ 
                            window.location='http://'+document.domain+'/supply/final/';
                        }
                        else {
                            window.location='http://'+document.domain+'/myuin/supplying/'; 
                        }
                    }).error(function(data, status, headers, config) {
                        window.console.log(status);
                }); 
